...
// Compiler bisa nge-generate panggilan IMPLISIT ke memset/memcpy sendiri
// (misal buat zero-init struct besar atau struct assignment/copy) walau kita
// gak pernah manggil keduanya secara eksplisit di kode ini. Karena gak ada
// libc sama sekali (--no-standard-libraries), dua simbol ini WAJIB ada
// sebagai fungsi global (bukan static) dengan nama & signature persis,
// biar linker bisa nyambungin panggilan implisit itu.
...
void *memcpy(void *dest, const void *src, __SIZE_TYPE__ size) {
  ...
  for (__SIZE_TYPE__ i = 0; i < size; i++) d[i] = s[i];
  return dest;
}

// Pengganti <stdlib.h> rand()/srand() — pola sama seperti rand_u32() di snek.c
...
void game_seed(unsigned int seed) {
  rng_state = seed;
}
